...
class DenseBlock(nn.Module):
	def __init__(self, in_features, out_features, batch_normal, drop_out):
		super().__init__()
...
class DenseNet(nn.Module):
	def __init__(self, layer1_dims, layer2_dims, K, initial="he_normal", exercise="European", batch_normal=False, drop_out=0):
		super().__init__()
		self.exercise = exercise
		self.K = K
		self.dense_layers1 = []
		self.skip_layers1 = []
		for i in range(len(layer1_dims) - 1):
			self.dense_layers1.append(
				DenseBlock(layer1_dims[i], layer1_dims[i+1])
			)
			self.skip_layers1.append(
				nn.Linear(layer1_dims[i], 1)
			)

		self.mul_layer = nn.Linear(layer1_dims[-1], 1)
		self.bias_layer = nn.Linear(layer1_dims[-1], 1)
		self.mul_layer2 = nn.Linear(layer1_dims[-1], 1)
		self.bias_layer2 = nn.Linear(layer1_dims[-1], 1)

		self.dense_layers2 = []
		self.skip_layers2 = []
		for i in range(len(layer2_dims) - 1):
			self.dense_layers2.append(
				DenseBlock(layer2_dims[i], layer2_dims[i+1])
			)
			self.skip_layers2.append(
				nn.Linear(layer2_dims[i], 1)
			)
		self.final_skip = nn.Linear(layer2_dims[-1], 1)
	...
	def forward(self, x_layer, t_layer, r_layer, q_layer, paras_layer):
		...
		mul = self.mul_layer(mid) + self.K / 10
		mul = nn.Softplus(mul) + 1e-6
		bias = self.bias_layer(mid)
		euro_mul_s = torch.exp(-q_layer*t_layer) if self.exercise == 'European' else 1
		euro_mul_k = torch.exp(-r_layer*t_layer) if self.exercise == 'European' else 1
		scaled = (torch.exp(x_layer)*euro_mul_s - self.K*euro_mul_k + bias*t_layer)/(torch.sqrt(t_layer+1e-8))/mul

		if self.cp == 'put':
			scaled = -scaled 
		singu = nn.Softplus(scaled) * (torch.sqrt(t_layer+1e-8)) * mul

		mul2 = self.mul_layer2(mid)+self.K/20
		mul2 = nn.Softplus(mul2) + 1e-6
		bias2 = self.bias_layer2(mid)*self.K/10 
		scaled2 = (torch.exp(x_layer)*euro_mul_s - self.K*euro_mul_k + bias2*t_layer)/(torch.sqrt(t_layer+1e-8))/mul2

		if self.cp == 'put':
			scaled2 = -scaled2 
		singu2 = nn.SiLU(scaled2) * (torch.sqrt(t_layer+1e-8)) * mul2
...
class Pricer:
	...
	def data_preparer(self, train_size=500000, test_size=10000):
		...
		for k,v in self.train_data.items():
			self.train_tensor[k] = torch.tensor(v, dtype=self.dtype)
		for k,v in self.test_data.items():
			self.test_tensor[k] = torch.tensor(v, dtype=self.dtype)
		self.y_grid = torch.tensor(self.y_grid, dtype=self.dtype)
		self.weight_grid = torch.tensor(self.weight_grid, dtype=self.dtype)
